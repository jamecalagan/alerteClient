...
- **interventions** : `id` (uuid), `client_id` (uuid), `reference`, `deviceType`, `brand`, `model`, `description`, `status`, `cost`, `solderestant`, `paymentStatus`, `commande`, `commande_effectuee`, `photos`, `label_photo`, `signatureIntervention`, `signature`, `createdAt`, `updatedAt`, `restitue`, `archived`, `archived_at`, `notifiedBy`, `notifiedat`, `notify_type`, `review_requested`, `review_responded`, `is_notified`, `imprimee`, `print_etiquette`, `password`, `chargeur`, `article_id`, `serial_number`, `loaned_item`, `loaned_item_returned`, `loaned_item_date`, `repair_proposal*` (made/price/status/method/comment/date), `estimate_min`, `estimate_max`, `estimate_type`, `is_estimate`, `estimate_accepted`, `estimate_accepted_at`, `devis_cost`, `info_note`, `repair_cause`, `repair_action`, `repair_duration`, `accept_screen_risk`. ⚠️ `id` et `client_id` sont de type `uuid` (confirmé par lecture directe du schéma) — ne jamais les écrire dans une colonne `bigint`/`integer` sans conversion explicite (voir bug `billing.intervention_id` corrigé ci-dessous).
- **orders** : `id`, `client_id`, `product`, `brand`, `model`, `price`, `deposit`, `paid`, `paid_at`, `saved`, `deleted`, `notified`, `signatureclient`, `printed`, `quantity`, `total`, `order_photos`, `ordered`, `received`, `recovered`, `source_quote_id`, `createdat`. ⚠️ `notified_method` n'existe PAS sur cette table (confirmé par erreur Supabase "Could not find the 'notified_method' column of 'orders' in the schema cache" le 2026-09-22) — ne jamais l'écrire, `notified` seul suffit.
