    ...
    if (is_estimate) {
      if (estimate_type === "PLAFOND") {
        return `<div class="cost"><strong>De ${CURRENCY(
          min
        )} à ${CURRENCY(max)} (plafond accepté${
      ...
      <div>
        ...
        ${
          !itv.is_estimate && itv.solderestant
            ? `<div class="costAcompte">Reste dû: ${CURRENCY(
                itv.solderestant
              )}</div>`
            : ""
        }
        ...
        <div class="terms-section">
          <p class="terms-text-bottom">
            Je soussigné(e), M.${clientInfo.name ||
              "________________________"}, certifie avoir pris connaissance que le
            matériel, qu'il soit réparé ou jugé non réparable, devra être récupéré
            dans un délai maximum de 30 jours. Au-delà de ce délai, le matériel
            sera considéré comme abandonné et pourra être détruit ou jeté sans
            recours possible.
          </p>
          <p class="terms-text">
          ...
          <p class="terms-text">
            Les anciens supports sont systématiquement restitués. Si le client ne
            souhaite pas récupérer son ancien support, celui-ci sera archivé avec le
            numéro de la fiche correspondant pour une durée de 3 mois avant
            destruction.
          </p>
          <p class="terms-text">
          ...
          <!-- ✅ AJOUT : commande non annulable -->
          <p class="alert-commande">
            IMPORTANT : Si l’intervention nécessite la commande d’une pièce ou d’un produit
          ...
          ${
            itv.accept_screen_risk
              ? `<div class="accept-risk">
                  J'accepte le risque de casse de l'écran tactile ou LCD.
                  Produit concerné : ${itv.deviceType}.
                </div>`
              : ""
          }
          <p class="info-recup">
...
`;
  ...
  const handlePrintBoth = async () => {
    try {
      ...
      const min =
        intervention.estimate_min != null ? Number(intervention.estimate_min) : null;
      const max =
        intervention.estimate_max != null ? Number(intervention.estimate_max) : null;
      ...
      const checkupSignatureHtml = checkupSig
        ? `<img src="${checkupSig}" style="width:200px;height:80px;" alt="Signature fiche de contrôle"/>`
...
<head>
  <style>
    ...
    .alert-commande { color: #b00000; font-weight: bold; font-size: 10px; margin-top: 6px; margin-bottom: 6px; }
...
${
  !intervention.is_estimate && intervention.partialPayment
    ? `<div class="cost">Acompte: ${CURRENCY(intervention.partialPayment)}</div>`
    : ""
}
${
  !intervention.is_estimate && intervention.solderestant
    ? `<div class="costAcompte">Montant restant dû: ${CURRENCY(intervention.solderestant)}</div>`
    : ""
}
...
<div class="terms-section">
  ...
  <div class="alert-commande">
    IMPORTANT : Si l’intervention nécessite la commande d’une pièce ou d’un produit (ex : dalle écran, clavier, batterie, etc.),
      ...
      <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={styles.infoBlock}>
            <View style={styles.infoRow}>
              <View style={styles.infoCol}>
                <Text style={styles.ficheNumber}>Fiche n° {clientInfo.ficheNumber}</Text>
              ...
              <View style={styles.infoCol}>
                ...
                <Text style={styles.infoLine}>
                  <Text style={styles.infoLabel}>N° série :</Text> {itv.reference}
                </Text>
                <Text style={styles.infoLine}>
            ...
            {itv.accept_screen_risk && (
              <Text style={styles.acceptRiskText}>
                J'accepte le risque de casse de l'écran tactile ou LCD. Produit concerné :{" "}
                {itv.deviceType}.
              </Text>
            )}
          </View>
            ...
            {itv.is_estimate ? (
              itv.estimate_type === "PLAFOND" ? (
                <Text style={styles.costText}>
                  ...
                  (plafond accepté
                  {itv.estimate_accepted_at ? ` le ${fmtDate(itv.estimate_accepted_at)}` : ""}
                  )
                </Text>
              ) : (
                <Text style={styles.costText}>
        ...
        <View style={styles.termsSection}>
          <Text style={styles.termsText}>
            ...
            avoir pris connaissance que le matériel, qu'il soit réparé ou jugé non réparable,
            devra être récupéré dans un délai maximum de 30 jours. Au-delà de ce délai, le
            matériel sera considéré comme abandonné et pourra être détruit ou jeté sans
            recours possible.
          </Text>
          <Text style={styles.termsText}>
            AVENIR INFORMATIQUE ne peut être tenu responsable de la perte de données sur disque
            dur ou tout autre support. Aucune réclamation ne sera prise en compte après le
            règlement de la facture.
          </Text>
          <Text style={styles.termsText}>
            Les anciens supports sont systématiquement restitués. Si le client ne souhaite pas
            récupérer son ancien support, celui-ci sera archivé avec le numéro de la fiche
          ...
          {/* ✅ AJOUT : commande non annulable (écran) */}
          ...
          <Text style={styles.termsText}>
            Responsabilité en cas de perte de données : Le client est seul responsable de ses données
            personnelles et/ou professionnelles et de leur sauvegarde régulière.
          </Text>
          <Text style={styles.termsText}>
            En cas de perte de données lors d’une prestation et/ou d’une manipulation, qu’elle soit d’origine
            logicielle ou matérielle, le client (particulier ou professionnel) ne pourra prétendre à aucune
            indemnisation, qu'il ait ou non une sauvegarde récente ou ancienne de ses données sur un autre support.
          </Text>
          <Text style={styles.termsText}>
            Toute intervention effectuée par le personnel d'AVENIR INFORMATIQUE se fait sous l’entière
            responsabilité du client. AVENIR INFORMATIQUE ne pourra en aucun cas être tenue responsable de la
            perte éventuelle d’informations. Le client reste donc seul responsable de ses données.
          </Text>
          <Text style={styles.termsText}>
          ...
          <View style={styles.signatureSection}>
            ...
            {itv?.signatureIntervention ? (
              <>
                <Image source={{ uri: itv.signatureIntervention }} style={styles.signatureImage} />
...
const styles = StyleSheet.create({
  ...
  // ✅ AJOUT : style clause commande non annulable (écran)
  orderNonCancelableText: {
    fontSize: 14,
    color: "#b00000",
    fontWeight: "bold",
    marginTop: 6,
