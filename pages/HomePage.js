...
const getOrderRemainingForClient = (orders = [], clientId) => {
    ...
    return orders
        ...
        .reduce((acc, o) => {
            ...
            const unit =
                typeof o.price === "number" ? o.price : _toNum(o.price);
            const total =
...
const __latestInterventionMs = (client) => {
    const list = Array.isArray(client?.interventions)
...
// --- Sticker / état commande pour l'encart du bas ---
// Gère plusieurs colonnes possibles + fallback propre
const getOrderSticker = (o) => {
    const statusStr = (o?.status ?? o?.etat ?? "")
        .toString()
        .trim()
        .toLowerCase();
    ...
    const recovered =
        ...
        o?.restitue === true ||
        o?.restituee === true ||
        statusStr.includes("restitu") ||
        statusStr.includes("livr") ||
        statusStr.includes("termine");

    const paid = o?.paid === true || isTruthy(o?.paid);

    const received =
        o?.received === true ||
        o?.commande_recue === true ||
        o?.recu === true ||
        statusStr.includes("reçue") ||
        statusStr.includes("recue");

    const passed =
        o?.ordered === true ||
        o?.commande_effectuee === true ||
        o?.passed === true ||
        statusStr.includes("pass") ||
        statusStr.includes("command");

    if (recovered) return { text: "Restituée", style: stylesNS.pillTerminee };
    if (paid) return { text: "Payée", style: stylesNS.pillPayee };
    if (received) return { text: "Reçue", style: stylesNS.pillEnCours };
    if (passed) return { text: "Passée", style: stylesNS.pillAttente };

    // ✅ fallback correct si rien n'est encore fait
    return { text: "À passer", style: stylesNS.pillNeutral };
};
...
export default function HomePage({ navigation, route, setUser }) {
    ...
    const openNotifyChooser = (client) => {
        const latest = __pickLatestActiveIntervention(
            client?.interventions || []
        );
        if (!latest) {
            Alert.alert(
    ...
    const loadOrdersInProgress = async () => {
        try {
            ...
            const sorted = (rows || [])
                ...
                .sort(
                    (a, b) =>
                        new Date(__coalesceDate(b)) -
                        new Date(__coalesceDate(a))
                );
      ...
      `
            ...
            const rows = (clientsData || [])
                .map((c) => {
                    ...
                    const ordersEnCours = orders.filter(
                        (o) => !o.paid || !o.saved
                    );
  ...
  notified
    ...
    `
            ...
            if (clientsData) {
                ...
                clientsToShow.sort(
                    (a, b) =>
                        __latestInterventionMs(b) - __latestInterventionMs(a)
                );
        ...
        `
            ...
            const enriched = combined.map((client) => {
                ...
                const totalAmountOngoing = interventions
                    .filter(
                        (i) =>
                            (i.solderestant || 0) > 0 && i.status !== "Récupéré"
                    )
    ...
    const filterClientsWithCommandeEnCours = async () => {
        try {
            const { data: unpaidOrders, error: orderError } = await supabase
                .from("orders")
                .select(
                    "id, client_id, paid, saved, price, deposit, product, brand, model, marque, modele, notified"
                )
                            ...
                            <View style={{ marginBottom: 20 }}>
                                ...
                                {isBannedMatch && (
                                    <Text style={styles.bannedHint}>
                                        ⚠️ Correspond à un client banni —
                                            ...
                                            return (
                                                <TouchableOpacity
                                                    ...
                                                    style={[
                                                        ...
                                                        isBanned &&
                                                            styles.sugRowBanned, // fond rosé si banni
                                                    ]}
                                                >
                                                    <Text
                                                        numberOfLines={1}
                                                    ...
                                                    {isBanned && (
                                                        <View
                                                            style={
                                                                styles.sugBadgeBanned
                                                            }
                                                        >
                                                            <Text
                                                                style={
                                                                    styles.sugBadgeBannedText
                                                                }
                                                            >
                                                                BANNI
                                                                {item?.ban_reason
                                    ...
                                    {showClients && (
                                        <FlatList
                                            ...
                                            renderItem={({ item, index }) => {
                                                ...
                                                const ordAgg =
                                                    summarizeClientOrders(
                                                        item.orders || []
                                                    );
                                                const ordDue =
                                                    ordAgg.restStandalone; // seulement les commandes "simples"
                                                const ordTotal =
                                                    ordAgg.totalStandalone;
                                                const ordDeposit =
                                                    ordAgg.depositStandalone;
                                                const hasIncludedOrder =
                                                    ordAgg.hasIncluded;
                                                    ...
                                                    >
                                                        <View
                                                            style={[
                                                                ...
                                                                isBanned &&
                                                                    styles.bannedRow, // ← fond rosé si banni
                                                            ]}
                                                        >
                                                            <View
                                                                style={
                                                            ...
                                                            {(() => {
                                                                ...
                                                                const hasActiveIntervention =
                                                                    !!li; // si li existe, on considère intervention prioritaire
...
<FlatList
  ...
  renderItem={({ item }) => {
    const hasInterv = item.interventionsEnCours.length > 0;
    const hasOrder = item.ordersEnCours.length > 0;
    ...
    return (
      ...
      >
        <Text style={{ fontWeight: "bold" }}>
          {item.client.name?.toUpperCase()} — Fiche {item.client.ficheNumber}
        </Text>

        {hasInterv && (
          <Text style={{ marginTop: 4 }}>
             Interventions en cours : {item.interventionsEnCours.length}
          </Text>
        )}
        ...
        {/* Bloc liens interventions / commandes */}
        {(hasInterv || hasOrder) && (
          <View
            style={{
              marginTop: 10,
          ...
          >
            ...
            >
              {/* Colonne gauche : Voir interventions (seulement si interventions) */}
              {hasInterv && (
                <TouchableOpacity
                  onPress={() => {
                  ...
                  style={{
                    flex: hasOrder ? 1 : 1, // garde la place normale
                    paddingVertical: 8,
                    justifyContent: "center",
                    alignItems: "center",
                  }}
              ...
              {/* Barre verticale seulement si on a les deux */}
              {hasInterv && hasOrder && (
                <View
                  style={{
                    width: 1,
              ...
              {/* Colonne droite : Voir commandes (seulement si commandes) */}
              {hasOrder && (
                <TouchableOpacity
                  onPress={() => {
                    setPopupVisible(false);
...
/>
                        ...
                        >
                            ...
                            >
                                ...
                                {/* Barre verticale centrale collée aux horizontales */}
                                <View
                                    style={{
                                        width: 1,
                                        backgroundColor: "#d1d5db",
