...
export default function OrdersPage({ route, navigation, order }) {
        ...
        const rows = (data || []).map((o) => {
          ...
          const unit =
            typeof o.price === "number"
              ? o.price
              : parseFloat((o.price ?? "0").toString().replace(",", ".")) || 0;
          const total =
          ...
          return {
            ...
            quantity: qty,
            total,
            include_in_intervention: toBool(o.include_in_intervention),
        ...
        if (data) {
          ...
          const unit =
            typeof data.price === "number"
              ? data.price
              : parseFloat((data.price ?? "0").toString().replace(",", ".")) || 0;
          const total =
            typeof data.total === "number" && !isNaN(data.total) ? data.total : unit * qty;
          ...
          setOrders([
            {
              ...
              quantity: qty,
              total,
              include_in_intervention: toBool(data.include_in_intervention),
  ...
  const handleCreateOrder = async () => {
    try {
      ...
      if (!included && !newOrder.price) {
        alert("Veuillez saisir un prix ou cochez “Coût inclus dans l’intervention”.");
        return;
      }
  ...
  const handleMarkAsPaid = (ord) => {
    ...
    const total = isIncluded ? 0 : (ord.total ?? (ord.price || 0) * (ord.quantity || 1));
  ...
  const candidateSingle = ["order_photo", "photo_url", "photo", "image_url", "image", "picture"];
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>
        {clientName ? `Commandes pour : ${clientName}` : `Commandes`}
      </Text>
      ...
      <View style={styles.formContainer}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            setNewOrder((o) => ({
              ...o,
              include_in_intervention: !o.include_in_intervention,
            }))
          }
        >
          <View
            style={[
              styles.checkbox,
              newOrder.include_in_intervention && styles.checkboxChecked,
            ]}
          >
            {newOrder.include_in_intervention && <Text style={styles.checkboxMark}>✓</Text>}
          </View>
          <Text style={styles.checkboxLabel}>Coût inclus dans l’intervention</Text>
        </TouchableOpacity>

        <TextInput
          style={[
            styles.input,
            newOrder.include_in_intervention && styles.inputDisabled,
          ]}
          placeholder="Prix unitaire (€)"
          placeholderTextColor="#000"
          keyboardType="numeric"
          value={newOrder.include_in_intervention ? "" : newOrder.price}
        ...
        <View style={styles.qtyRow}>
          <TouchableOpacity
            style={[
              styles.qtyButton,
              newOrder.include_in_intervention && styles.buttonDisabled,
            ]}
            onPress={() => {
          ...
          <TextInput
            ...
            keyboardType="numeric"
            inputMode="numeric"
            value={newOrder.include_in_intervention ? "" : newOrder.quantity}
            onChangeText={(t) => {
        ...
        <View style={{ alignItems: "center" }}>
          <TouchableOpacity style={styles.addButton} onPress={handleCreateOrder}>
            <Text style={styles.addButtonText}>Ajouter une commande</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <FlatList
        ref={listRef}
        data={orders}
        extraData={{ expandedOrders, editingIds, editMap, uploadingOrderId }}
        ...
        renderItem={({ item, index }) => {
          ...
          const remaining = isIncluded ? 0 : Math.max(0, total - (item.deposit || 0));
            ...
            <View style={styles.orderCard}>
              <TouchableOpacity activeOpacity={0.9} onPress={() => toggleExpand(item.id)}>
                <View style={[styles.tableRow, rowBg]}>
                  ...
                  <Text style={[styles.rowText, styles.colUnit]}>
                    {isIncluded ? "—" : `${fmtMoney(unit)} €`}
                  </Text>
                  <Text style={[styles.rowText, styles.colTotal]}>
                    {isIncluded ? "Inclus" : `${fmtMoney(total)} €`}
                  </Text>
                  <Text style={[styles.rowText, styles.colDeposit]}>
                  ...
                  <Text
                    style={[
                      styles.rowText,
                      styles.colRemaining,
                      item.paid ? styles.greenText : styles.redText,
                    ]}
                  >
                    {isIncluded ? "0,00 €" : `${fmtMoney(remaining)} €`}
                  </Text>
                  <Text style={[styles.rowText, styles.colPaid]}>
                    {item.paid ? "Oui" : "Non"}
                  </Text>
                </View>
              </TouchableOpacity>
                ...
                <View style={styles.expandArea}>
                  {!editing ? (
                    <>
                      <View style={styles.kvBlock}>
                        {renderKV("Produit", item.product || "-")}
                        {renderKV("Marque", item.brand || "-")}
                        {renderKV("Modèle", item.model || "-")}
                        {renderKV("N° de série", item.serial || "-")}
                        {renderKV("Prix unitaire", isIncluded ? "—" : `${fmtMoney(unit)} €`)}
                        {renderKV("Quantité", isIncluded ? "—" : String(qty))}
                        {renderKV(
                          "Total",
                          isIncluded ? "0,00 € (inclus)" : `${fmtMoney(total)} €`,
                          true
                        )}
                        {renderKV("Acompte", `${fmtMoney(item.deposit)} €`)}
                        {renderKV(
                          "Montant restant dû",
                          isIncluded ? "0,00 € (inclus)" : `${fmtMoney(remaining)} €`,
                          true
                        )}

                        {item.paid_at &&
                          renderKV("Payée le", new Date(item.paid_at).toLocaleDateString())}
                        ...
                        {renderKV("Commande passée", item.ordered ? "Oui" : "Non")}
                        {renderKV("Commande reçue", item.received ? "Oui" : "Non")}
                        {renderKV("Récupérée client", item.recovered ? "Oui" : "Non")}
                        {renderKV("Notifiée", item.notified ? "Oui" : "Non")}
                        {renderKV("Sauvegardée", item.saved ? "Oui" : "Non")}
                      </View>
                    ...
                    <View style={styles.editBlock}>
                      ...
                      >
                        <View
                          style={[
                            styles.checkbox,
                            editVals.include_in_intervention && styles.checkboxChecked,
                          ]}
                        >
                          {editVals.include_in_intervention && <Text style={styles.checkboxMark}>✓</Text>}
                        </View>
                        <Text style={styles.checkboxLabel}>Coût inclus dans l’intervention</Text>
                      </TouchableOpacity>
                      ...
                      <TextInput
                        style={[
                          styles.input,
                          editVals.include_in_intervention && styles.inputDisabled,
                        ]}
                        placeholder="Prix unitaire (€)"
                        placeholderTextColor="#000"
                        keyboardType="numeric"
                        inputMode="decimal"
                        value={editVals.include_in_intervention ? "" : editVals.price}
                        onChangeText={(t) =>
                          updateEditField(item.id, "price", t.replace(/[^0-9.,]/g, ""))
                        }
                        editable={!editVals.include_in_intervention}
                      />
                      ...
                      <View style={styles.qtyRow}>
                        <TouchableOpacity
                          style={[
                            styles.qtyButton,
                            editVals.include_in_intervention && styles.buttonDisabled,
                          ]}
                          onPress={() => changeQty(item.id, -1)}
                          disabled={editVals.include_in_intervention}
                        >
                          <Text style={styles.qtyButtonText}>−</Text>
                        </TouchableOpacity>
                        <TextInput
                          ...
                          keyboardType="numeric"
                          inputMode="numeric"
                          value={editVals.include_in_intervention ? "" : editVals.quantity}
                          onChangeText={(t) =>
                            updateEditField(item.id, "quantity", (t ?? "").replace(/[^0-9]/g, ""))
                          }
                          editable={!editVals.include_in_intervention}
                        />
                        <TouchableOpacity
                      ...
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          updateEditField(item.id, "deposit", t.replace(/[^0-9.,]/g, ""))
                        }
                      />
                          ...
                          onLongPress={() =>
                            Alert.alert("Supprimer la photo", "Voulez-vous supprimer cette photo ?", [
                              { text: "Annuler", style: "cancel" },
                              {
                                text: "Supprimer",
                                style: "destructive",
                                onPress: () => deleteOnePhoto(item, paths[idx]),
                              },
                            ])
                          }
                        >
                  ...
                  <View style={styles.actionsRow}>
                    <TouchableOpacity
                      style={[
                        styles.squareButton,
                        uploadingOrderId === item.id && { opacity: 0.6 },
                      ]}
                    ...
                    >
                      {uploadingOrderId === item.id ? (
                        ...
                        <Text style={styles.squareButtonText}>Ajouter photo</Text>
                      )}
                    </TouchableOpacity>
                      ...
                      onPress={() => {
                        navigation.navigate("CommandePreviewPage", {
                          order: {
                            ...
                            price: isIncluded ? 0 : item.price,
                            total: isIncluded ? 0 : item.total,
                    ...
                    >
                      <Text style={styles.squareButtonText}>Imprimer</Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                      style={[styles.squareButton, item.ordered && styles.squareButtonDisabled]}
                      onPress={() => !item.ordered && handleMarkAsOrdered(item)}
                      disabled={item.ordered}
                    >
                      <Text style={[styles.squareButtonText, item.ordered && styles.squareButtonTextDisabled]}>
                        {item.ordered ? "Commande passée" : "Marquer passée"}
                      </Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                      style={[styles.squareButton, item.received && styles.squareButtonDisabled]}
                      onPress={() => !item.received && handleMarkAsReceived(item)}
                      disabled={item.received}
                    >
                      <Text style={[styles.squareButtonText, item.received && styles.squareButtonTextDisabled]}>
                        {item.received ? "Commande reçue" : "Marquer reçue"}
                      </Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                      style={[styles.squareButton, item.paid && styles.squareButtonDisabled]}
                      onPress={() => !item.paid && handleMarkAsPaid(item)}
                      disabled={item.paid}
                    >
                      <Text style={[styles.squareButtonText, item.paid && styles.squareButtonTextDisabled]}>
                        {item.paid ? "Payée" : "Marquer payée"}
                      </Text>
                    </TouchableOpacity>

                    {(item.billing?.length ?? 0) === 0 ? (
                      <TouchableOpacity
                        style={[styles.squareButton, isIncluded && styles.squareButtonDisabled]}
                        onPress={() =>
                          !isIncluded &&
                          navigation.navigate("BillingPage", {
                            expressData: {
                              ...
                              brand: item.brand,
                              model: item.model,
                              price: String(item.total ?? (item.price || 0) * (item.quantity || 1)),
                        ...
                        disabled={isIncluded}
                      >
                        <Text style={[styles.squareButtonText, isIncluded && styles.squareButtonTextDisabled]}>
                          {isIncluded ? "Inclus (pas de facture)" : "Créer facture"}
                        </Text>
                      </TouchableOpacity>
                    ) : (
                    ...
                    <TouchableOpacity
                      style={[styles.squareButton, item.recovered && styles.squareButtonDisabled]}
                      onPress={() => !item.recovered && handleMarkAsRecovered(item)}
                      disabled={item.recovered}
                    >
                      <Text style={[styles.squareButtonText, item.recovered && styles.squareButtonTextDisabled]}>
                        {item.recovered ? "Récupérée" : "Marquer récupérée"}
                      </Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                      style={[styles.squareButton, item.saved && styles.squareButtonDisabled]}
                      disabled={item.saved}
                      onPress={() => handleSaveOrder(item)}
                    >
                      <Text style={[styles.squareButtonText, item.saved && styles.squareButtonTextDisabled]}>
                        {item.saved ? "Sauvegardée" : "Sauvegarder"}
                      </Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      style={[
                        styles.squareButton,
                        (!item.received || item.notified) && styles.squareButtonDisabled,
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          ...
                          (!item.received || item.notified) && styles.squareButtonTextDisabled,
                        ]}
                      >
                        {item.notified ? "Notifiée" : "Notifier"}
                      </Text>
                    </TouchableOpacity>
              ...
              {item.saved && !isExpanded && (
                <TouchableOpacity style={styles.openRowButton} onPress={() => toggleExpand(item.id)}>
                  <Text style={styles.openRowButtonText}>Ouvrir</Text>
                </TouchableOpacity>
              )}
            </View>
          );
...
const styles = StyleSheet.create({
  ...
  rowStatus: { fontSize: 11, fontWeight: "700", color: "#6b7280", marginTop: 2 },
