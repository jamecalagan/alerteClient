...
export default function OrdersPage({ route, navigation, order }) {
    ...
    const saveEdit = async (id) => {
        try {
            ...
            if (!v.product || price <= 0) {
                Alert.alert(
                    "Champs manquants",
                    "Renseignez au minimum le produit et un prix unitaire valide."
                );
                return;
            }
    ...
    // 🧠 IMPORTANT : charge la liste au montage et quand le clientId change
    useEffect(() => {
        loadOrders();
    }, [clientId]);

    // 🧠 IMPORTANT : recharge quand on revient sur la page (focus)
    useEffect(() => {
    ...
    // 🧠 IMPORTANT : recharge quand QuoteEditPage envoie { refreshAt }
    useEffect(() => {
        if (route.params?.refreshAt) {
            loadOrders();
            // (option) nettoyer les params :
            // navigation.setParams({ refreshAt: undefined, focusId: undefined });
        }
    }, [route.params?.refreshAt]);

    const toBool = (v) => v === true || v === "true" || v === 1;

    // 🔁 Charge commandes : si clientId → commandes du client ; sinon → toutes les commandes
// 🔁 Charge commandes : si clientId → commandes du client ; sinon → toutes les commandes
const loadOrders = async () => {
  try {
    const focusId = route.params?.focusId ? String(route.params.focusId) : null;

    // Cas 1 : on a un clientId → on charge SES commandes
    if (clientId) {
      ...
      const rows = (data || []).map((o) => {
        const qty  = Number.isFinite(o.quantity) ? o.quantity : parseInt(o.quantity ?? 1, 10) || 1;
        const unit = typeof o.price === "number" ? o.price : parseFloat((o.price ?? "0").toString().replace(",", ".")) || 0;
        const total = typeof o.total === "number" && !isNaN(o.total) ? o.total : unit * qty;
        return {
          ...o,
          quantity: qty,
          total,
          notified: o.notified === true || o.notified === "true" || o.notified === 1,
          received: o.received === true || o.received === "true" || o.received === 1,
          paid:     o.paid === true || o.paid === "true" || o.paid === 1,
          ordered:  o.ordered === true || o.ordered === "true" || o.ordered === 1,
          recovered:o.recovered === true || o.recovered === "true" || o.recovered === 1,
          saved:    o.saved === true || o.saved === "true" || o.saved === 1,
        };
      });
      setOrders(rows);
      return;
    ...
    // Cas 2 : pas de clientId mais on vient d’une conversion → on montre UNIQUEMENT cette commande
    if (focusId) {
      const { data, error } = await supabase
        .from("orders")
        .select("*, billing(id)")
      ...
      if (data) {
        const qty  = Number.isFinite(data.quantity) ? data.quantity : parseInt(data.quantity ?? 1, 10) || 1;
        const unit = typeof data.price === "number" ? data.price : parseFloat((data.price ?? "0").toString().replace(",", ".")) || 0;
        const total = typeof data.total === "number" && !isNaN(data.total) ? data.total : unit * qty;

        setOrders([{
          ...data,
          quantity: qty,
          total,
          notified: data.notified === true || data.notified === "true" || data.notified === 1,
          received: data.received === true || data.received === "true" || data.received === 1,
          paid:     data.paid === true || data.paid === "true" || data.paid === 1,
          ordered:  data.ordered === true || data.ordered === "true" || data.ordered === 1,
          recovered:data.recovered === true || data.recovered === "true" || data.recovered === 1,
          saved:    data.saved === true || data.saved === "true" || data.saved === 1,
        }]);

        // Option : si tu veux afficher le nom/phone en haut quand tu arrives par focusId
    ...
    // Cas 3 : ni clientId ni focusId → ne rien lister
    setOrders([]);
  } catch (e) {
    ...
    const handleCreateOrder = async () => {
        try {
            ...
            const payload = {
                ...
                paid: false,
                client_id: clientId || null, // ← autorise création même sans clientId si besoin
            };
    ...
    const handleMarkAsPaid = (ord) => {
        const remaining =
            (ord.total ?? ord.price * (ord.quantity || 1)) - (ord.deposit || 0);
        Alert.alert(
            "Paiement complet",
...
<Text style={styles.header}>
  {clientName ? `Créer une commande pour: ${clientName}` : `Commandes`}
</Text>
                        ...
                        <View style={styles.orderCard}>
                            ...
                            {urls.length > 0 && (
                                <Text style={styles.thumbHint}>
                                    Touchez pour agrandir • Restez appuyé pour
                                    supprimer
                                </Text>
                            )}
