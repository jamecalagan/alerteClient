...
const [isSigning, setIsSigning] = useState(false);
    ...
    useEffect(() => {
        if (!interventionId || !isValidUUID(interventionId)) {
            console.error(
                "Erreur : interventionId n'est pas un UUID valide ou est manquant."
            );
            return;
        }
    ...
    const handleCaptureAndConfirmSignature = async () => {
        try {
            ...
            console.error(
                "Erreur lors de la confirmation de la signature :",
                error
            );
            Alert.alert(
                "Erreur",
  ...
  `;
    const handleCaptureAndPrint = async () => {
        try {
            if (!signature) {
                Alert.alert(
                    "Erreur",
                    "Veuillez fournir une signature avant d'imprimer."
                );
                return;
            }
            ...
            console.error(
                "Erreur lors de la sauvegarde ou de la navigation :",
                error
            );
            Alert.alert(
                "Erreur",
    ...
    const handleSaveAndNavigateToPrint = async () => {
        try {
            ...
            console.error(
                "Erreur lors de la sauvegarde et de la navigation :",
                error
            );
            Alert.alert(
                "Erreur",
...
<ScrollView
  ...
  scrollEnabled={!isSigning} // ✅ bloque le scroll pendant la signature
>
        ...
        />
            <Text style={styles.fixedText}>
                ...
                , atteste avoir récupéré le matériel mentionné et reconnais
                avoir été informé(e) des conditions suivantes :{"\n\n"}
                <Text style={styles.boldText}>
                    1. Garantie commerciale – durée et portée :
                </Text>
                {"\n"}Le matériel restitué est couvert par une garantie
                commerciale d’une durée de trois (3) mois à compter de la date
                de restitution.
                {"\n"}Cette garantie ne s’applique qu’à la panne initialement
                identifiée et réparée. Toute autre anomalie ou défaillance
                ultérieure, non directement liée à l’intervention initiale, est
                expressément exclue de cette garantie.
                {"\n"}Les réparations consécutives à une oxydation ou à une
                exposition à un liquide ne sont pas couvertes, en raison du
                caractère aléatoire et non maîtrisable de ce type de
                dysfonctionnement.
                {"\n\n"}
                <Text style={styles.boldText}>2. Délais de réclamation :</Text>
                {"\n"}Le client dispose d’un délai de dix (10) jours calendaires
                à compter de la date de restitution pour formuler toute
                réclamation.
                {"\n"}Aucune demande ne pourra être recevable au-delà de ce
                délai, sauf disposition légale contraire.
                {"\n\n"}
                ...
                {"\n"}• ou d’intervention effectuée par un tiers non autorisé
                par AVENIR INFORMATIQUE.
                {"\n\n"}
                <Text style={styles.boldText}>
                    4. Responsabilité relative aux données personnelles :
                </Text>
                {"\n"}Il appartient exclusivement au client de procéder à la
                sauvegarde préalable de ses données.
                {"\n"}AVENIR INFORMATIQUE décline toute responsabilité en cas de
                perte totale ou partielle de données, quelle qu’en soit la
                cause, y compris lors de l’intervention technique.
                {"\n\n"}Fait à : Drancy
                {"\n"}Le : {new Date().toLocaleDateString()}
                {"\n"}Signature du client :
            </Text>

        {/* ✅ Signature avec hauteur fixe */}
